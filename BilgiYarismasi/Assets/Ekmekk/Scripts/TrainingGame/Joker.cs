...
public class Joker : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        txt_correct.text = PlayerPrefs.GetInt("nickname-correct"+User.GetInstance.nickname, 10).ToString();
        txt_bomb.text = PlayerPrefs.GetInt("nickname-bomb"+User.GetInstance.nickname, 10).ToString();
        txt_pass.text = PlayerPrefs.GetInt("nickname-pass"+User.GetInstance.nickname, 10).ToString();
    ...
    void Bomb()
    {
        ...
        int count = PlayerPrefs.GetInt("nickname-bomb"+User.GetInstance.nickname, 10);

        if (count > 0)
        {
            count--;
            PlayerPrefs.SetInt("nickname-bomb"+User.GetInstance.nickname, count);
            ...
            answerController.answers[answer].GetOut();
    ...
    void Pass()
    {
        ...
        int count = PlayerPrefs.GetInt("nickname-pass"+User.GetInstance.nickname, 10);
        if (count > 0)
        {
            count--;
            PlayerPrefs.SetInt("nickname-pass"+User.GetInstance.nickname, count);
    ...
    void Correct()
    {
        ...
        int count = PlayerPrefs.GetInt("nickname-correct"+User.GetInstance.nickname, 10);
        if (count > 0)
        {
            count--;
            txt_correct.text = count.ToString();
            PlayerPrefs.SetInt("nickname-correct"+User.GetInstance.nickname, count);
            int correct = Convert.ToInt16(FindObjectOfType<TrainingManager>().currentQuestion.id);
    ...
    public void LockButton(bool isUnlocked)
    {
        btn_bomb.enabled = isUnlocked;
        btn_correct.enabled = isUnlocked;
        btn_pass.enabled = isUnlocked;
    }
}