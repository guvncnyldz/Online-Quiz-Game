...
public class Answer : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        btn_answer = GetComponentInParent<Button>();
    ...
    public void GetOut()
    {
        ...
        img_answer.rectTransform.DOAnchorPos(new Vector2(ANSWEROUTPOSX, 0), 0.25f)
            .SetEase(Ease.Linear);

        img_choice.rectTransform.DOAnchorPos(new Vector2(CHOICEOUTPOSX, 0), 0.25f)
            .SetEase(Ease.Linear);
    }
    ...
    {
        btn_answer.enabled = false;
        img_answer.rectTransform.DOAnchorPos(new Vector2(ANSWEROUTPOSX, 0), 0.25f).OnComplete(() =>
            {
                txt_answer.text = answer;
                img_answer.rectTransform.DOAnchorPos(new Vector2(ANSWERINPOSX, 0), 0.25f).SetEase(Ease.Linear)
                    .SetDelay(0.25f);
            }
        ).SetEase(Ease.Linear);

        img_choice.rectTransform.DOAnchorPos(new Vector2(CHOICEOUTPOSX, 0), 0.25f).OnComplete(() =>
            {
                img_choice.sprite = defaulChoice;
                img_choice.rectTransform.DOAnchorPos(new Vector2(CHOICEINPOSYX, 0), 0.25f).SetEase(Ease.Linear)
                    .SetDelay(0.25f).OnComplete(() => { btn_answer.enabled = true; });
            }
        ).SetEase(Ease.Linear);
    }
